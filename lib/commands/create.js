...
module.exports = function (program) {
  ...
  program
    ...
    .description('Create a boilerplate Geoportal 2.x project')
    ...
    .action(function(shortName, options) {
      ...
      var boilerplateUrl = (options.boilerplate) ? options.boilerplate : config.GEOPORTAL_BOILERPLATE;
      ...
      // do some token replaces to rename the geoportal project
      replaceInConfig(/geoportal-boilerplate|geoportal-open-museum/g, shortName);

      // Session secret
      var secret = util.secret();
      if (fs.existsSync('./lib/modules/geoportal-express/index.js')) {
        util.replaceInFiles(['./lib/modules/geoportal-express/index.js'], /secret: undefined/, 'secret: \'' + secret + '\'');
      }
      ...
      if (options.install || options.setup) {
        ...
        // & we're not married to a very old version of Geoportal
        exec('npm update');
        ...
        prompt.get({
          ...
          exec('echo "' + result.password + '" | node app.js geoportal-users:add admin admin');
